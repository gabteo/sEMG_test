...
def animate(i, dataList, ser):
    ...
    time.sleep(0.004)
    ser.write(b'g')                                     # Transmit the char 'g' to receive the Arduino data point
    arduinoData_string2 = ser.readline().decode('ascii') # Decode receive Arduino data as a formatted string
    #print(i)                                           # 'i' is a incrementing variable based upon frames = x argument

    try:
        arduinoData_float = float(arduinoData_string)   # Convert to float
        arduinoData_float2 = float(arduinoData_string2)   # Convert to float
    ...
    dataList = dataList[-250:]                           # Fix the list size so that the animation plot 'window' is x number of points
    
    ax.clear()                                          # Clear last data frame
    ax.plot(dataList)                                   # Plot new data frame
    
    ax.set_ylim([1600, 2000])                              # Set Y axis limit of plot
